...
const withDiscriminator = (
  ...
  const discriminatedValues = keys(pickBy(
    discriminator.mapping,
    (i) => i === schema.$ref
  ));
  
  if (discriminatedValues.length > 0) {
    const discriminatedValuesType = f.createUnionTypeNode(discriminatedValues.map(val => f.createLiteralTypeNode(f.createStringLiteral(val))));
    ...
    if (spec && isSchemaObject(spec) && spec.properties) {
      ...
      if (
        ...
        discriminatedValues.includes(property.enum[0]) &&
